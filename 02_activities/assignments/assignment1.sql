...
select cp.product_id, cp.vendor_id, cp.market_date, cp.customer_id, cp.quantity, cp.cost_to_customer_per_qty, cp.transaction_time, quantity * cost_to_customer_per_qty as price
...
select c.customer_id, c.customer_first_name, c.customer_last_name, sum(cp.quantity * cp.cost_to_customer_per_qty ) as total_amount 
from customer_purchases cp
inner JOIN
customer c
...
insert into new_vendor values (10, 'Thomass Superfood Store', 'Fresh Focused store','Thomas', 'Rosenthal');
...
select customer_id, sum(quantity*cost_to_customer_per_qty) as total_amount
from customer_purchases
